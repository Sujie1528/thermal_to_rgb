...
logging.basicConfig(
    ...
    filemode='a',
    format=
    '%(asctime)s %(levelname)-8s [%(filename)s:%(name)s:%(lineno)d] %(message)s',
...
class pithermalcam:
    ...
    _colormap_list = [
        'jet', 'bwr', 'seismic', 'coolwarm', 'PiYG_r', 'tab10', 'tab20',
        'gnuplot2', 'brg', 'gray'
    ]
    _interpolation_list = [
    ...
    _interpolation_list_name = [
        'Nearest', 'Inter Linear', 'Inter Area', 'Inter Cubic',
        'Inter Lanczos4', 'Pure Scipy', 'Scipy/CV2 Mixed'
    ]
    ...
    def _pull_raw_image(self):
        ...
        try:
            ...
            # Ballpark temperature values in Celsius
            self._temp_min = 20
            self._temp_max = 45
            
            self._raw_image = self._temps_to_rescaled_uints(
                self._raw_image, self._temp_min, self._temp_max)
    ...
    def _add_image_text(self):
        ...
        cv2.putText(self._image, text, (30, 18), cv2.FONT_HERSHEY_SIMPLEX, .4,
                    (255, 255, 255), 1)
        ...
        if self._file_saved_notification_start is not None and (
            ...
            cv2.putText(self._image, 'Snapshot Saved!', (300, 300),
    ...
    def add_customized_text(self, text):
        ...
        cv2.putText(self._image, text, (300, 300), cv2.FONT_HERSHEY_SIMPLEX, .8,
                    (255, 255, 255), 2)
        time.sleep(0.1)
    ...
    def _set_click_keyboard_events(self):
        ...
        if key == ord("s"):  # If s is chosen, save an image to filec
            ...
            self.filter_image = not self.filter_image
                ...
                "t"):  # If t is chosen cycle the units used for Temperature
            self.use_f = not self.use_f
        elif key == ord(
                "u"):  # If t is chosen cycle the units used for temperature
    ...
    def save_image(self):
        ...
        fname = self.output_folder + 'pic_' + dt.datetime.now().strftime(
            '%Y-%m-%d_%H-%M-%S') + '.jpg'
    ...
    def _temps_to_rescaled_uints(self, f, Tmin, Tmax):
        ...
        # Set all the values outside the range to the min and max values
        f[f < Tmin] = Tmin
        f[f > Tmax] = Tmax
